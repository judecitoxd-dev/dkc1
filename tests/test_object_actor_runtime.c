...
int main(void){
    ...
    assert(actor.active&&actor.scheduler.type_id[3u]==DK1_ANIMATED_RENDER_OBJECT_TYPE);
    assert(dk1_object_actor_step(&actor,&rom,&base,transform,style,0u,
                                 100u,80,8,8,&interaction));
    assert(interaction&&!actor.active&&actor.interactions==1u);
    assert(actor.callback_verified&&actor.callback_pc==DK1_ANIMATED_RENDER_CALLBACK_PC);
    ...
    assert(actor.scheduler.type_id[3u]==0u);
